...
class Booking {
    async getBookings(req, res) {
        try {
            const { propertyId, fromDate, toDate, page = 1, limit = 10, scope, status } = req.query
            const bookings = await BookingService.getBookings({ fromDate, propertyId, toDate, limit, page, status, scope });
