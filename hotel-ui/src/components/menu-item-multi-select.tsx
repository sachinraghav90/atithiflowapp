...
export function MenuItemMultiSelect({
    ...
    const filteredItems = useMemo(() => {
        return normalizedItems.filter((item) => {
            ...
            return !isDisabled || value.some(v => String(v) === itemId);
        });
    ...
    const selectedItems = useMemo(() => {
        ...
        return normalizedItems.filter(item => 
            value.some(v => String(v) === String(item.id) || String(v) === String(item.label))
        );
                    ...
                    >
                        ...
                        <CommandGroup>
                            {filteredItems.map((item) => {
                                ...
                                const isSelected = value.some(v => String(v) === itemId);
                                    ...
                                    <CommandItem
                                        ...
                                        onSelect={() => {
                                            ...
                                            const newValue = isSelected 
                                                ? value.filter(v => String(v) !== itemId)
                                                : [...value, finalValue];
                                                
                                            onSelect(newValue);
                                        }}
                                        className={cn(
                                            "flex items-center cursor-pointer py-1.5 px-2 text-sm rounded-sm transition-all justify-between",
                                            showFullText && "items-start whitespace-normal",
                                            isSelected ? "bg-primary/10 text-primary" : "hover:bg-primary/5 hover:text-primary"
                                        )}
                                    >
